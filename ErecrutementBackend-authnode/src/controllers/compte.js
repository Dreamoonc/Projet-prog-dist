...
const createDemandeur = async (req, res) => {
  try {
    ...
    const numeroCarteIdentite = req.body.numeroCarteIdentite
    const situationFamiliale = req.body.situationFamiliale
    const nombreEnfants = req.body.nombreEnfants
    const competences = req.body.competences
    const niveauEtudes = req.body.niveauEtudes
    const langues = req.body.langues
    const handicap = req.body.handicap
    const experienceProfessionnelles =req.body.experienceProfessionnelles
    const password= await PasswordHashed.hashPassword(req.body.password)
...
// delete by id employeur or demandeur
 ...
 const getUsersByRole = async (req, res) => {
  try {
    ...
    // Validate the role against the predefined roles
    if (role === 'Employeur' || role === 'Demandeur' || role === 'admin') {
...
module.exports={createAdmin, createDemandeur, ConfirmDemandeurAndEmployeur, createEmployeur, getEmployeurOrDemandeur,
  forgetPassword, resetPassword, getById, deleteById, getAllComptes, DesactiverActiverAccount, getUsersByRole}
